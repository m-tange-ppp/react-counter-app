...
export default function Counter(props) {
    ...
    return (
        <div className={`flex flex-col items-center justify-center gap-2 p-2 w-[100%] sm:w-[calc(50%-0.75rem)] border-neutral-900 rounded-xl bg-gradient-to-r ${props.flagBg ? `from-emerald-400 to-blue-600` : `from-yellow-600 to-red-700 `} transition-shadow duration-300 hover:ring-4 ring-pink-500 relative`}>
            <p> <span className="inline-block w-14 text-center">{props.operation === "add" ? "Add" : "Multiply"}</span> by <span className="text-xl bg-neutral-200 border-4 border-dashed border-black inline-block p-2 w-12 text-center  rounded-lg">{props.rate}</span> starting from <span className="text-xl bg-neutral-200 border-4 border-black border-double inline-block p-2 w-12 text-center rounded-lg">{props.start}</span></p>
